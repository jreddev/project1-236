...
void Relation::toString(){
    if (!tuples.empty()){
        ...
        for (auto t : tuples) {
            ...
            std::cout << header->attributes[tupleSize-1] << "='" << t.values[tupleSize-1] << "'"<< std::endl;
        }
...
/*void Relation::toString(){
    if (!tuples.empty()){
        ...
        while (it != tuples.end()) {
            for(int i = 0; i < tupleSize-1; i++) {
                std::cout << header->attributes[i] << "=" << (*it).values[i] << ", ";
            }
            std::cout << header->attributes[tupleSize-1] << "='" << (*it).values[tupleSize-1] << "'"<< std::endl;
            it++;
        }
    }
}*/
